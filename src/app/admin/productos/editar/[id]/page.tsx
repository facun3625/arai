...
export default function EditarProductoPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [catsRes, attrsRes, prodRes] = await Promise.all([
                ...
                if (prodData) {
                    setFormData({
                        ...
                        categories: prodData.categories?.map((c: any) => c.id) || [],
                        price: prodData.price?.toString() || "0",
                        compareAtPrice: prodData.compareAtPrice?.toString() || "",
                        stock: prodData.stock?.toString() || "0",
                        featuredImage: prodData.featuredImage || "",
                        images: JSON.parse(prodData.images || "[]"),
                        weight: prodData.weight?.toString() || "",
                        width: prodData.width?.toString() || "",
                        height: prodData.height?.toString() || "",
                        length: prodData.length?.toString() || "",
                        addons: JSON.parse(prodData.addons || "[]")
                    });

                    if (prodData.type === "VARIABLE" && prodData.variants) {
                        const parsedVariants = prodData.variants.map((v: any) => ({
                            ...v,
                        ...
                        // Extract selected attributes and terms from variations
                        if (parsedVariants.length > 0) {
                            const attrIds: string[] = [];
                            const termsMap: { [key: string]: string[] } = {};

                            // Get attribute names from first variant
                            const attrNames = Object.keys(parsedVariants[0].attributes);

                            attrNames.forEach(aName => {
                                const attr = attrsData.find((a: any) => a.name === aName);
                                if (attr) {
                                    attrIds.push(attr.id);
                                    // Get unique terms used for this attribute across all variants
                                    const termsUsed = Array.from(new Set(parsedVariants.map((v: any) => v.attributes[aName])));
                                    termsMap[attr.id] = termsUsed as string[];
                                }
                            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        console.log("SUBMITTING PRODUCT DATA:", { ...formData, addons: formData.addons, variants: formData.type === "VARIABLE" ? variants : [] });
        try {
