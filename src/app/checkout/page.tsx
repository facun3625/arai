...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        if (currentStep === 2 && !ocaEnabled && !acordarEnabled) {
            setSelectedShippingMethod('coordinar');
            setSelectedShipping(0);
        }
    }, [currentStep, ocaEnabled, acordarEnabled]);

    // OCA Shipping Logic - Trigger when entering step 2 (Shipping)
    useEffect(() => {
        const zip = shippingAddress.zipCode;
        if (currentStep === 2 && zip && zip.length === 4 && ocaEnabled) {
                                    ...
                                    <section>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {isCalculatingShipping ? (
                                                ...
                                                <>
                                                    {/* OCA options only if enabled and NOT BLOCKED */}
                                                    {ocaEnabled && activeRestriction?.type !== 'BLOCK_SHIPPING' && (
                                                        <>
                                                            {ocaQuote && (
                                                    ...
                                                    {/* Contactanos: se muestra si ningún método está habilitado */}
                                                    {!ocaEnabled && !acordarEnabled && !(freeShippingThreshold > 0 && subtotal >= freeShippingThreshold) && (
                                                        <div className="p-6 bg-primary/5 border border-primary/20 rounded-2xl flex items-start gap-4">
                                                            <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                                                                <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current text-primary" xmlns="http://www.w3.org/2000/svg">
                                                            ...
                                                            <div className="flex-1">
                                                                <p className="text-sm font-semibold text-gray-900 mb-1">Coordinamos el envío con vos</p>
                                                                <p className="text-xs text-gray-500 leading-relaxed mb-3">Una vez recibido tu pedido, nos pondremos en contacto para coordinar el envío según tu ubicación.</p>
                                                                {whatsappNumber && (
                                                                    <a
                                                                        href={`https://wa.me/${whatsappNumber}?text=Hola!%20Quiero%20consultar%20sobre%20el%20env%C3%ADo%20de%20mi%20pedido.`}
                                                                        target="_blank"
                                                                        rel="noopener noreferrer"
                                                                        className="inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#20b858] text-white text-xs font-semibold px-4 py-2 rounded-xl transition-colors"
                                                                    >
